...
class WalmartParser:
    ...
    def __post_init__(self) -> None:
        self.logger = logging.getLogger("dealwatch.stores.walmart.parser")
    ...
    def _extract_product_key(url: str, product: dict[str, Any]) -> str | None:
        match = _ITEM_ID_RE.search(urlsplit(url).path)
        if match is not None:
            return match.group("item_id")
        for field_name in ("sku", "productID", "productId", "gtin13"):
    ...
    def _build_unit_price_info(self, *, title: str, product: dict[str, Any]) -> dict[str, str | float]:
        ...
        gtin = self._clean_text(product.get("gtin13"))
        if gtin:
            info["gtin"] = gtin
    ...
    def _extract_brand_name(brand: Any) -> str | None:
        if isinstance(brand, dict):
            return WalmartParser._clean_text(brand.get("name"))
        return WalmartParser._clean_text(brand)
